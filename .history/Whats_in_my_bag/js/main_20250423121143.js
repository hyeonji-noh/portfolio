...
document.addEventListener("DOMContentLoaded", function () {
  ...
  const rowConfigs = {
    row1: { range: [0, 9], color: "#FF6E4E" },   // 0부터 9번째까지 (10개의 셀)
    row2: { range: [0, 9], color: "#3BFBA8" },   // 0부터 9번째까지 (10개의 셀)
    row3: { range: [0, 5], color: "#3BFBA8" },   // 0부터 5번째까지 (6개의 셀)
    row4: { range: [0, 3], color: "#3BFBA8" }    // 0부터 3번째까지 (4개의 셀)
  };

  const rows = document.querySelectorAll(".bingo_board .row");

  rows.forEach((row) => {
    // row 클래스에서 'row1', 'row2' 등을 찾기
