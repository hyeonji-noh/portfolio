...
document.addEventListener("DOMContentLoaded", function () {
  ...
  rows.forEach((row, rowIndex) => {
    const rowName = row.classList.contains("row1") ? "row1"
                   : row.classList.contains("row2") ? "row2"
                   : row.classList.contains("row3") ? "row3"
                   : row.classList.contains("row4") ? "row4"
                   : null;
    ...
    cells.forEach((cell, index) => {
      cell.addEventListener("click", function () {
        ...
        if (rowName === "row1" && index <= 9) {
          color = "#FF6E4E";
        } else if (rowName === "row2") {
          if (index <= 8) color = "#3BFBA8";
          else if (index === 9) color = "#FF6E4E";
        } else if (rowName === "row3" && index <= 5) {
          color = "#3BFBA8";
        } else if (rowName === "row4" && index <= 3) {
          color = "#3BFBA8";
        }

        if (color) {
          this.style.setProperty("background-color", color, "important");
          console.log(`${rowName} - index ${index} → colored ${color}`);
        } else {
          console.log(`${rowName} - index ${index} → not target`);
        }
      });
    });
  });
