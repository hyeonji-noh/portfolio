...
document.addEventListener("DOMContentLoaded", () => {
  ...
  setTimeout(() => {
    // 1번째 줄 (모든 셀을 주황으로)
    const firstRow = rows[0].querySelectorAll(".cell");
    firstRow.forEach(cell => cell.classList.add("orange"));

    // 2번째 줄 10번째 셀 주황
    const secondRow = rows[1].querySelectorAll(".cell");
    ...
    // 3번째 줄 6번째 셀 민트
    const thirdRow = rows[2].querySelectorAll(".cell");
    thirdRow.forEach((cell, index) => {
      if (index < 6) {
        cell.classList.add("mint");
      }
    });
    ...
    // 4번째 줄 4번째 셀 민트
    const fourthRow = rows[3].querySelectorAll(".cell");
    fourthRow.forEach((cell, index) => {
      if (index < 4) {
        cell.classList.add("mint");
      }
    });
  }, 100); // 페이지 로드 후 0.1초 뒤에 실행
});
})