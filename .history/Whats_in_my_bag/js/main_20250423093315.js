...
document.addEventListener("DOMContentLoaded", function () {
  const rowConfigs = {
    row1: { range: [0, 10], color: "#FF6E4E" },           // 0~9 (10칸 전체) 오렌지
    row2: { range: [0, 9], color: "#3BFBA8", last: "#FF6E4E" }, // 0~8 민트, 9 오렌지
    row3: { range: [0, 6], color: "#3BFBA8" },             // 0~5 민트
    row4: { range: [0, 4], color: "#3BFBA8" }              // 0~3 민트
  };

  document.querySelectorAll(".bingo_board .row").forEach(row => {
    const rowClass = [...row.classList].find(cls => cls.startsWith("row"));
