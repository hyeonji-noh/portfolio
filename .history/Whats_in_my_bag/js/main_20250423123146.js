  ...
  $(window).on('scroll', function() {
    ...
    if (windowBottom > sectionTop && windowTop < sectionBottom) {
      ...
      $('.row2 .cell').each(function(index) {
        setTimeout(() => {
          if (index < 10) {
            $(this).addClass('highlight-mint');  // 우선 민트색으로 설정
          }
        }, (10 + index) * 300); // 10개 셀 후 0.3초씩 지연
      });

      // 두 번째 줄 마지막 셀만 주황색 칠하기
      setTimeout(() => {
        $('.row2 .cell').last().addClass('highlight-orange');
      }, (20 + 10) * 300); // 마지막 셀은 전체 애니메이션 이후 0.3초 후에 주황색으로 설정
      ...
      // 애니메이션이 끝나면 더 이상 실행되지 않도록 이벤트 리스너 제거
      $(window).off('scroll');
    }
  });
});
