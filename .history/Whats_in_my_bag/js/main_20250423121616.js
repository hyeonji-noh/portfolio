...
document.addEventListener("DOMContentLoaded", function () {
  ...
  // .row 클래스를 가진 모든 요소 선택
  const rows = document.querySelectorAll(".bingo_board .row");

  rows.forEach((row) => {
    // 각 .row에서 클래스 이름을 추출 (예: row1, row2, row3, row4)
    ...
    cells.forEach((cell, index) => {
      ...
      cell.addEventListener("click", function () {
        console.log(`클릭된 셀: rowClass=${rowClass}, index=${index}`);
        ...
        // row2: 10열만 주황색, 나머지 1~9열은 민트색
        else if (rowClass === "row2") {
          if (index === 9) {
            this.style.backgroundColor = config.color; // 10열 주황색
        ...
        // row3: 1~6열까지 민트색
          ...
          if (index >= 0 && index <= 5) {
            this.style.backgroundColor = config.color; // 민트색
          }
        }
