...
$(document).ready(function() {
  ...
  // 색칠이 이미 완료된 상태를 방지하기 위한 변수
  let isSectionVisited = false;

  // 섹션에 진입했을 때 색칠 시작
  $(window).on('scroll', function() {
    if (isSectionVisited) return; // 이미 섹션에 방문했다면 색칠하지 않음
    ...
    if (windowBottom > sectionTop && windowTop < sectionBottom) {
      // 섹션에 들어왔을 때만 애니메이션 실행
      // 애니메이션이 실행되기 전에 모든 셀을 초기화 (색상 제거)
      $('.cell').removeClass('highlight-orange highlight-mint');

      // 첫 번째 줄 주황색 칠하기 (차례대로 하나씩 색칠)
