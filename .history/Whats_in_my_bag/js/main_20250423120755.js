...
document.addEventListener("DOMContentLoaded", function () {
  ...
  const rows = document.querySelectorAll(".bingo_board .row").forEach(row => {
    console.log(row.classList);  // DOM이 로드되기 전에 로그 찍기
  });
  ...
  rows.forEach((row) => {
    ...
    console.log("Row class found:", rowClass);  // 확인

    if (!rowClass) {
      console.error("Row class not found");
      return;  // rowClass가 없으면 처리하지 않음
    }
