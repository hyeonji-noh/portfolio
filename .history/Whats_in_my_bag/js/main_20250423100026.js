    ...
    cells.forEach((cell, index) => {
      cell.addEventListener("click", function () {
        ...
        if (rowClass === "row2" && index === 9 && config.last) {
          console.log('Setting last color for row2');
          this.style.backgroundColor = config.last; // row2의 마지막 셀에 색을 지정
        } else if (index >= config.range[0] && index < config.range[1]) {
          console.log(`Setting color ${config.color} for index ${index}`);
          this.style.backgroundColor = config.color; // 범위 내 셀에 색을 지정
        }
      });
    });
  });
