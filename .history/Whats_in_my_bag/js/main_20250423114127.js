  ...
  rows.forEach(row => {
    ...
    cells.forEach((cell, index) => {
      cell.addEventListener("click", function () {
        ...
        if (rowClass === "row1" && index <= 9) {
          color = "#FF6E4E";
        } else if (rowClass === "row2") {
          if (index <= 8) color = "#3BFBA8";
          else if (index === 9) color = "#FF6E4E";
        } else if (rowClass === "row3" && index <= 5) {
          color = "#3BFBA8";
        } else if (rowClass === "row4" && index <= 3) {
          color = "#3BFBA8";
        }

        if (color) {
          this.style.setProperty("background-color", color, "important");
          console.log(`Clicked ${rowClass} cell ${index} → colored ${color}`);
        } else {
          console.log(`Clicked ${rowClass} cell ${index} → no color`);
        }
      });
    });
  });
